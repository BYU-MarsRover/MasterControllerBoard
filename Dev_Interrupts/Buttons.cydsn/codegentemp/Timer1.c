...
void Timer1_Init(void)
{
    ...
    /* Set other values from customizer */
        ...
        #if (Timer1__COUNT_DOWN == Timer1_TC_COUNTER_MODE)
            Timer1_WriteCounter(Timer1_TC_PERIOD_VALUE );
        #endif  /* (Timer1__COUNT_DOWN == Timer1_TC_COUNTER_MODE) */
...
void Timer1_WriteCompare(uint32 compare)
{
        ...
        uint32 currentMode;
    #endif /* (Timer1_CY_TCPWM_V2) */

    #if (Timer1_CY_TCPWM_V2)
        currentMode = ((Timer1_CONTROL_REG & Timer1_UPDOWN_MASK) >> Timer1_UPDOWN_SHIFT);

        if (Timer1__COUNT_DOWN == currentMode)
        {
            Timer1_COMP_CAP_REG = ((compare + 1u) & Timer1_16BIT_MASK);
        }
        else if (Timer1__COUNT_UP == currentMode)
        {
            Timer1_COMP_CAP_REG = ((compare - 1u) & Timer1_16BIT_MASK);
        }
        else
...
uint32 Timer1_ReadCompare(void)
{
        ...
        if (Timer1__COUNT_DOWN == currentMode)
        {
            regVal = ((Timer1_COMP_CAP_REG - 1u) & Timer1_16BIT_MASK);
        }
        else if (Timer1__COUNT_UP == currentMode)
        {
            regVal = ((Timer1_COMP_CAP_REG + 1u) & Timer1_16BIT_MASK);
        ...
        {
            regVal = (Timer1_COMP_CAP_REG & Timer1_16BIT_MASK);
        }
        return (regVal);
    #else
...
void Timer1_WriteCompareBuf(uint32 compareBuf)
{
        ...
        if (Timer1__COUNT_DOWN == currentMode)
        {
            Timer1_COMP_CAP_BUF_REG = ((compareBuf + 1u) & Timer1_16BIT_MASK);
        }
        else if (Timer1__COUNT_UP == currentMode)
        {
            Timer1_COMP_CAP_BUF_REG = ((compareBuf - 1u) & Timer1_16BIT_MASK);
...
uint32 Timer1_ReadCompareBuf(void)
{
        ...
        if (Timer1__COUNT_DOWN == currentMode)
        {
            regVal = ((Timer1_COMP_CAP_BUF_REG - 1u) & Timer1_16BIT_MASK);
        }
        else if (Timer1__COUNT_UP == currentMode)
        {
            regVal = ((Timer1_COMP_CAP_BUF_REG + 1u) & Timer1_16BIT_MASK);
        ...
        {
            regVal = (Timer1_COMP_CAP_BUF_REG & Timer1_16BIT_MASK);
        }
        return (regVal);
    #else
...
*   Values:
*     - Timer1_CMD_CAPTURE    - Trigger Capture command
*     - Timer1_CMD_RELOAD     - Trigger Reload command
*     - Timer1_CMD_STOP       - Trigger Stop command
*     - Timer1_CMD_START      - Trigger Start command
*
* Return:
*  None
*
