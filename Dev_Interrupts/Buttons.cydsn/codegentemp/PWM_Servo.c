...
void PWM_Servo_Init(void)
{
    ...
    /* Set other values from customizer */
        ...
        #if (PWM_Servo__COUNT_DOWN == PWM_Servo_TC_COUNTER_MODE)
            PWM_Servo_WriteCounter(PWM_Servo_TC_PERIOD_VALUE );
        #endif  /* (PWM_Servo__COUNT_DOWN == PWM_Servo_TC_COUNTER_MODE) */
...
void PWM_Servo_WriteCompare(uint32 compare)
{
        ...
        uint32 currentMode;
    #endif /* (PWM_Servo_CY_TCPWM_V2) */

    #if (PWM_Servo_CY_TCPWM_V2)
        currentMode = ((PWM_Servo_CONTROL_REG & PWM_Servo_UPDOWN_MASK) >> PWM_Servo_UPDOWN_SHIFT);

        if (PWM_Servo__COUNT_DOWN == currentMode)
        {
            PWM_Servo_COMP_CAP_REG = ((compare + 1u) & PWM_Servo_16BIT_MASK);
        }
        else if (PWM_Servo__COUNT_UP == currentMode)
        {
            PWM_Servo_COMP_CAP_REG = ((compare - 1u) & PWM_Servo_16BIT_MASK);
        }
        else
...
uint32 PWM_Servo_ReadCompare(void)
{
        ...
        if (PWM_Servo__COUNT_DOWN == currentMode)
        {
            regVal = ((PWM_Servo_COMP_CAP_REG - 1u) & PWM_Servo_16BIT_MASK);
        }
        else if (PWM_Servo__COUNT_UP == currentMode)
        {
            regVal = ((PWM_Servo_COMP_CAP_REG + 1u) & PWM_Servo_16BIT_MASK);
        ...
        {
            regVal = (PWM_Servo_COMP_CAP_REG & PWM_Servo_16BIT_MASK);
        }
        return (regVal);
    #else
...
void PWM_Servo_WriteCompareBuf(uint32 compareBuf)
{
        ...
        if (PWM_Servo__COUNT_DOWN == currentMode)
        {
            PWM_Servo_COMP_CAP_BUF_REG = ((compareBuf + 1u) & PWM_Servo_16BIT_MASK);
        }
        else if (PWM_Servo__COUNT_UP == currentMode)
        {
            PWM_Servo_COMP_CAP_BUF_REG = ((compareBuf - 1u) & PWM_Servo_16BIT_MASK);
...
uint32 PWM_Servo_ReadCompareBuf(void)
{
        ...
        if (PWM_Servo__COUNT_DOWN == currentMode)
        {
            regVal = ((PWM_Servo_COMP_CAP_BUF_REG - 1u) & PWM_Servo_16BIT_MASK);
        }
        else if (PWM_Servo__COUNT_UP == currentMode)
        {
            regVal = ((PWM_Servo_COMP_CAP_BUF_REG + 1u) & PWM_Servo_16BIT_MASK);
        ...
        {
            regVal = (PWM_Servo_COMP_CAP_BUF_REG & PWM_Servo_16BIT_MASK);
        }
        return (regVal);
    #else
...
*   Values:
*     - PWM_Servo_CMD_CAPTURE    - Trigger Capture command
*     - PWM_Servo_CMD_RELOAD     - Trigger Reload command
*     - PWM_Servo_CMD_STOP       - Trigger Stop command
*     - PWM_Servo_CMD_START      - Trigger Start command
*
* Return:
*  None
*
