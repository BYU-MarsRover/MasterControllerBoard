...
// Notes for eventual roverlink.c file:
// Have private variable that keeps track of Destination IP Address Target, or query it on every write.
// This will assure that when the dstAddr is changed, the Ip Address is adjusted as well.
// This is inevitable: Ip Addresses must be unique on the network, ... ah, wait, ah..... brain stuck
// UDP only sends to a specific Ip Address, unless we do Broadcasting/Unicast. I designed the RoverLink
// protocol from the start thinking that this would be the case (doing broadcasting), and thus IP addresses 
// were/are irrelevant, every node on the network gets every packet. But then, the PSOCs must process all data.
// Thus, we want to have it such that the PSOCS only get .... oh duh, Socket Port assignments.
// So we have three things going on: IP Address, IP Port, and RoverLink Alias.
// IP Address must be unique on each Wiznet and IP Camera. Static assignment if no UDP broadcast, each node must know list.
// 		If UDP Broadcasting, no static Ip addresses needed, more freedom, at risk of broadcasting breaking.
// 		Lets make a compromise: Basestation sends to specific Ip Addresses, or MACs to be even more low-level robust
// 			Rover broadcasts to all nodes using UDP broadcast.
// 			This would allow any basestation to monitor any/all rover telemetry.
// 			PSOC only accepts commands from a specific Basestation Ip address (How do I do that?)
// IP Ports are assigned to target nodes. There source port numbers, which I've made ephemeral (non-reply).
// 		We are only concerned with the destination UDP port
// RoverLink Alias 



// Ok, so here is how it works, having added BROADCAST_ALIAS to the RoverLink.xml
// A Basestation transmits to the target Rover board and that board's UDP Port.
// The Rover board in question accepts the packet if it is the correct port, and if possible, from the correct IP address (Try UIPR/UPORTR)
