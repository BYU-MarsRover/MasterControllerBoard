...
***************************************/
    ...
    volatile uint8 GPS_UART_rxBufferInternal[GPS_UART_RX_BUFFER_SIZE];
    ...
    *
    * Global Variables:
    *  Look into GPS_UART_SpiInit for description.
    *
    *******************************************************************************/
    uint32 GPS_UART_SpiUartReadRxData(void)
    {
        ...
        #if(GPS_UART_CHECK_RX_SW_BUFFER)
        {
            if(GPS_UART_rxBufferHead != GPS_UART_rxBufferTail)
            {
                ...
                if(GPS_UART_RX_BUFFER_SIZE == locTail)
                {
                    locTail = 0u;
                }
        ...
        #else
        {
            rxData = GPS_UART_RX_FIFO_RD_REG; /* Read data from RX FIFO */
        }
        #endif
    ...
    *******************************************************************************/
    uint32 GPS_UART_SpiUartGetRxBufferSize(void)
    {
        ...
        #if(GPS_UART_CHECK_RX_SW_BUFFER)
        {
            ...
            {
                size = (locHead + (GPS_UART_RX_BUFFER_SIZE - GPS_UART_rxBufferTail));
            }
        }
        #else
        {
    ...
    void GPS_UART_SpiUartWriteTxData(uint32 txData)
    {
        ...
        #if(GPS_UART_CHECK_TX_SW_BUFFER)
        {
            ...
            if((GPS_UART_txBufferHead == GPS_UART_txBufferTail) &&
               (GPS_UART_FIFO_SIZE != GPS_UART_GET_TX_FIFO_ENTRIES))
            {
            ...
            else
            {
                ...
                intSourceMask  = GPS_UART_INTR_TX_NOT_FULL;
                intSourceMask |= GPS_UART_GetTxInterruptMode();
                GPS_UART_SpiUartEnableIntTx(intSourceMask);
            }
        }
        #else
        {
    ...
    *******************************************************************************/
    void GPS_UART_SpiUartClearTxBuffer(void)
    {
        ...
        #if(GPS_UART_CHECK_TX_SW_BUFFER)
        {
            intSourceMask = GPS_UART_SpiUartDisableIntTx();
            ...
            intSourceMask &= (uint32) ~GPS_UART_INTR_TX_NOT_FULL;
